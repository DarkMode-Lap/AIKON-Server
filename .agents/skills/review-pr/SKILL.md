...
**Rule priority**: `CLAUDE.md` > `.claude/rules/**`
...
Always cite a specific source in the rationale (e.g. `.claude/rules/logging.md`, `Kotlin: prefer val over var`).
...
Do not modify any code. Record the refutation rationale for Step 4.
...
|---|----------|------|---------|-----------|--------|
| 1 | alice | Foo.kt:12 | ✅ VALID | .claude/rules/logging.md | Auto-fixed (abc1234) |
| 2 | bob | Bar.kt:34 | ❌ INVALID | .claude/rules/exception.md | Skipped |
| 3 | alice | Baz.kt:56 | ⚠️ PARTIAL | - | PENDING |
```

## Step 5 — Push Commits

If any VALID fixes were committed in Step 3, push them:
...
Post an inline reply for each comment. Always quote `comment_id` to prevent shell injection.
