...
class AvatarImagePromptProvider {
    fun getPrompt(
        ...
        return when (style) {
            Style.ENHANCED -> buildEnhancedPrompt(subject, ageHint)
    ...
    private fun buildEnhancedPrompt(
        ...
        """
        첨부된 사진 속 $subject 특정 화풍이나 캐릭터로 바꾸지 말고, 원본 사진을 더 멋있고 예쁜 프리미엄 프로필 사진처럼 보정해줘.
        $ageHint
        인물의 얼굴형, 눈코입, 헤어스타일, 표정, 나이대, 전체적인 생김새는 원본 사진과 최대한 동일하게 유지해줘.
        조명은 화려하지만 과하지 않은 시네마틱 소프트 조명으로 정리하고,
        피부톤은 자연스럽고 건강하게 보이도록 보정하되 실제 피부 질감은 유지해줘.
        색감은 선명하고 고급스럽게, 대비와 채도는 SNS 프로필이나 브랜드 화보처럼 세련되게 조정해줘.
        배경은 인물을 돋보이게 하는 감각적인 스튜디오 또는 도시적인 무드의 흐림 배경으로 구성해줘.
        눈빛과 표정은 더 또렷하고 자신감 있게 보이도록 살리고,
        이목구비는 원본보다 더 또렷하고 매력적인 비율로 보정해서 확실히 더 잘생기고 예쁘게 표현해줘.
        전체 결과물은 원본 인물의 정체성을 유지한 화려한 고급 프로필/화보 사진 퀄리티로 완성해줘.
        애니메이션, 수채화, 동물 캐릭터, 전통 의상 등 별도의 스타일 변환은 하지 마.
        """.trimIndent()
    ...
    private fun buildZootopiaPrompt(
        ...
        원본 인물의 눈매, 얼굴 비율, 헤어 실루엣, 액세서리 특징은 동물 캐릭터 디자인 안에 자연스럽게 반영해줘.
        배경은 다양한 동물들이 공존하는 활기차고 화려한 동물 도시(Zootopia 시내)로 설정하고,
        네온사인, 마천루, 분주한 거리 풍경을 담되 인물이 가장 먼저 보이도록 구도를 잡아줘.
        색감은 밝고 채도가 높으며 생동감 넘치는 디즈니 애니메이션 특유의 스타일로,
        털, 귀, 꼬리, 의상 소재는 부드럽고 입체감 있게 렌더링해줘.
        캐릭터의 표정과 눈빛은 원본 인물의 분위기와 감정을 그대로 반영하고,
        이목구비는 원본보다 더 또렷하고 매력적인 비율로 보정해서 확실히 더 잘생기고 예쁜 캐릭터로 표현해줘.
        전체 퀄리티는 디즈니 공식 영화 장면 수준으로 렌더링해줘.
        얼굴이 과도하게 바뀌거나 전혀 다른 캐릭터처럼 보이지 않게 원본과의 유사성을 유지해줘.
        """.trimIndent()
    ...
    private fun buildHanbokPrompt(
        ...
        한복의 비단 질감, 자수, 옷고름, 주름, 장신구를 섬세하고 고급스럽게 묘사해줘.
        구도는 인물이 중심이 되는 우아한 전신 또는 반신 초상화로 잡고,
        전체적으로 고풍스럽고 우아한 한국 전통 회화 일러스트 스타일로 표현해줘.
        피부는 깨끗하고 화사하게, 이목구비는 더 또렷하고 아름답게 보정해서 원본보다 확실히 매력적으로 표현해줘.
        인물의 얼굴형과 생김새, 눈코입, 헤어라인, 표정의 특징은 원본과 최대한 동일하게 유지해줘.
        장식 요소가 얼굴을 가리거나 원본 인물의 인상을 바꾸지 않도록 자연스럽게 배치해줘.
        """.trimIndent()
    ...
    private fun buildPixarPrompt(
        ...
        머리카락의 형태, 눈매, 미소, 얼굴 윤곽은 원본 인물을 알아볼 수 있게 캐릭터 디자인에 반영해줘.
        의상은 원본의 분위기를 참고해 밝고 세련된 3D 애니메이션 의상으로 재해석해줘.
        배경은 픽사 영화에 나올 법한 화려하고 디테일한 환경으로 구성하되 인물이 또렷하게 돋보이게 하고,
        눈동자 하이라이트, 피부 질감, 머리카락, 옷감의 재질감을 정교하게 렌더링해줘.
        전체 퀄리티는 픽사 영화 공식 포스터 수준으로 렌더링해줘.
        이목구비는 원본보다 더 또렷하고 매력적인 비율로 보정해서 확실히 더 잘생기고 예쁜 캐릭터로 표현해줘.
        인물의 헤어 색상, 얼굴 특징, 개성과 표정은 최대한 원본과 유사하게 유지하고,
        손, 치아, 눈동자 같은 디테일이 어색하게 뭉개지지 않도록 완성도를 높여줘.
        """.trimIndent()
    ...
    private fun buildGhibliPrompt(
        ...
        인물 주변에는 자연스러운 바람감과 은은한 생활감이 느껴지는 작은 디테일을 더해줘.
        얼굴 윤곽, 눈매, 코와 입의 위치, 헤어스타일, 표정은 원본 사진을 기반으로 최대한 유사하게 유지해줘.
        피부는 깨끗하고 화사하게, 이목구비는 더 또렷하고 사랑스럽게 보정해서 원본보다 확실히 예쁘게 표현해줘.
        전체적으로 평화롭고 따뜻하며 향수를 자극하는 지브리 특유의 감성을 담고,
        배경이 과하게 복잡해져 인물의 얼굴 인상이 흐려지지 않게 균형을 잡아줘.
        """.trimIndent()
    ...
    private fun buildLightArtPrompt(
        ...
        종이 질감, 물 번짐, 가장자리의 부드러운 그라데이션이 자연스럽게 드러나게 해줘.
        인물의 눈매, 얼굴형, 헤어스타일, 표정, 포즈는 원본 사진을 기반으로 유지하고,
        얼굴의 핵심 특징은 흐릿하게 사라지지 않도록 선명한 초점으로 살려줘.
        피부는 맑고 화사하게, 이목구비는 더 또렷하고 아름답게 보정해서 원본보다 확실히 매력적으로 표현해줘.
        전체적으로 갤러리에 걸릴 법한 섬세하고 아름다운 수채화 작품 느낌으로 완성하되,
        추상적인 장식이 과해져 인물 식별성이 낮아지지 않게 균형을 맞춰줘.
        """.trimIndent()
}
