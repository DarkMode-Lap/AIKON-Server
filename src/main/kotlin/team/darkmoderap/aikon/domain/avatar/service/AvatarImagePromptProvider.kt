...
class AvatarImagePromptProvider {
    ...
    private fun buildZootopiaPrompt(
        ...
        """
        첨부된 사진 속 $subject 현대 도시에서 살아가는 의인화된 동물 캐릭터로 변환해줘.
        $ageHint
        인물의 전체적인 분위기, 표정, 헤어스타일을 참고해서 가장 잘 어울리는 동물 종을 선택하고,
        세련되고 현대적인 의상을 입혀줘.
        배경은 다양한 동물들이 공존하는 활기차고 화려한 대도시로 설정하고,
        네온사인, 마천루, 분주한 거리 풍경을 담아줘.
        색감은 밝고 채도가 높으며 생동감 넘치는 컬러풀한 애니메이션 스타일로,
        캐릭터의 표정과 눈빛은 원본 인물의 분위기와 감정을 그대로 반영해줘.
        전체 퀄리티는 극장용 3D 애니메이션 영화 장면 수준으로 렌더링해줘.
        """.trimIndent()
    ...
    private fun buildPixarPrompt(
        ...
        """
        첨부된 사진 속 $subject 고품질 3D 애니메이션 캐릭터 스타일로 변환해줘.
        $ageHint
        눈은 실제보다 크고 둥글고 표현력 있게, 볼은 발그레하고 통통하게,
        전체적인 신체 비율은 살짝 과장된 귀엽고 사랑스러운 비율로 조정해줘.
        피부에는 빛이 은은하게 투과되는 SSS 효과를 적용해 생동감 있게 표현하고,
        조명은 따뜻하고 부드러운 시네마틱 3점 조명으로 설정해줘.
        배경은 다채롭고 디테일한 동화 같은 환경으로 구성하고,
        전체 퀄리티는 극장용 3D 애니메이션 공식 포스터 수준으로 렌더링해줘.
