...
class AvatarImagePromptProvider {
    fun getPrompt(style: Style): String =
        when (style) {
            Style.STUDIO -> {
                """
                첨부된 사진 속 인물을 고급 전문 스튜디오 포트레이트 사진 스타일로 변환해줘.
            ...
            Style.ZOOTOPIA -> {
                """
                첨부된 사진 속 인물을 디즈니 애니메이션 쥬토피아(Zootopia) 스타일의
                의인화된 동물 캐릭터로 변환해줘.
            ...
            Style.TRADITIONAL_HANBOK -> {
                """
                첨부된 사진 속 인물에게 아름다운 한국 전통 한복을 입혀서 새롭게 그려줘.
                저고리는 선명한 색상(붉은색, 연두색, 하늘색 등)에 繊細한 꽃 자수 장식을 더하고,
                치마는 풍성하게 흘러내리는 실크 소재의 아이보리 또는 옥색으로 표현해줘.
                머리는 전통 쪽머리 또는 댕기를 드리운 스타일로 꾸미고 비녀와 노리개도 추가해줘.
                배경은 조선시대 궁궐 정원으로, 만개한 벚꽃나무, 석등, 기와 처마, 연못을 배치해줘.
                조명은 따뜻한 황금빛 노을로 설정하고, 전체적으로 고풍스럽고 우아한 한국 전통 회화 일러스트 스타일로 표현해줘.
            ...
            Style.DISNEY_PIXAR -> {
                """
                첨부된 사진 속 인물을 픽사(Pixar) 스튜디오의 3D 애니메이션 캐릭터 스타일로 변환해줘.
            ...
            Style.GHIBLI -> {
                """
                첨부된 사진 속 인물을 스튜디오 지브리(Studio Ghibli) 특유의 손그림 애니메이션 스타일로 다시 그려줘.
            ...
            Style.LIGHT_ART -> {
                """
                첨부된 사진 속 인물을 고급 파인아트 수채화(Watercolor) 일러스트 스타일로 변환해줘.
